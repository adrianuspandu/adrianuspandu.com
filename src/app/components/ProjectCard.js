...
export default function ProjectCard({ name }) {
  ...
  const projectTags = projectData.tags.map((tag) => {
    return (
      <span
        key={tag}
        className="p-3 border-2 border-black rounded-md text-sm font-bold"
      >
        {tag}
      </span>
    );
  ...
  return (
    <div className="w-sm flex flex-col gap-6 border-2 border-black rounded-md shadow-[4px_4px_black] py-3 px-4">
      <Image
        className="object-cover w-full h-80 rounded-md"
        src={projectPlaceholder}
        alt={name}
      />
      <h3 className="font-bold text-3xl">{projectData.title}</h3>
      <p className="text-xl text-grayText">{projectData.description}</p>
